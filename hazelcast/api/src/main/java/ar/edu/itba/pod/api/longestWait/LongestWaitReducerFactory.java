...
public class LongestWaitReducerFactory implements ReducerFactory<Long, LongestWaitReducerValue, LongestWaitReducerValue> {
    ...
    @Override
    public Reducer<LongestWaitReducerValue, LongestWaitReducerValue> newReducer(Long aLong) {
        return new Reducer<>() {
            ...
            public void reduce(LongestWaitReducerValue value) {
                if (max == null ||   // Desempata por orden alfabetico ascendiente (si la nueva tiene menor, la guardo)
                        ...
                        value.waitTimeMillis().equals(max.waitTimeMillis()) && value.doLocationName().compareTo(max.doLocationName()) < 0) {
                    max = value;
                }
            }
